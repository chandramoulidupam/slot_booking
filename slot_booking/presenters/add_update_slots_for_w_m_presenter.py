...
class AddUpdateSlotsPresenterImplementation(PresenterInterface):
    ...
    def alter_slots_for_washing_machine_returns_slots_details(self, list_of_slot_dtos):
        list_of_added_or_updated_slot_details = []
        for each_slot in list_of_slot_dtos:
            each_slot_details = self._get_each_slot_details(each_slot)
            list_of_added_or_updated_slot_details.append(each_slot_details)
            count_of_avilable_slots = len(list_of_added_or_updated_slot_details)
            list_of_added_or_updated_slot_details_response = {
                "list_of_added_or_updated_slot_details": list_of_added_or_updated_slot_details,
                "number_of_slots_allocated_to_washing_machine": count_of_avilable_slots
            }
            return response.HttpResponse(json.dumps(list_of_added_or_updated_slot_details_response), status=200)

    def raise_exception_for_invalid_inputs(self):
        response_object = response.HttpResponse(json.dumps(
            {
                "response": INVALID_INPUTS_ARE_GIVEN[0],
                "http_status_code": INVALID_INPUTS_ARE_GIVEN[1],
                "res_status": 404
            }
        ), status=404)
        return response_object
