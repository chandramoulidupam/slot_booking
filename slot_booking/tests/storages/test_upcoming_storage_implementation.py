...
class TestUpcomingSlotsStorager:
    def test_upcoming_slots_with_valid_details_results_list_of_avilable_slots(self, slots, washing_machines):
        username = "user1"
        date = "2020-07-25"
        expected_slot_dtos = []
        storage = SlotStorageImplementation()
        # storage.validate_washing_machine_number.return_value = True
        response_slot_dto = storage.list_of_upcoming_slot_dtos(username=username,date=date)
        # Assert
