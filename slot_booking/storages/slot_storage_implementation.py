...
from slot_booking.constants.custom_exceptions import NoSlotsAreAvilable
...
class SlotStorageImplementation(StorageInterface):
    ...
    def get_each_user_slot_dto(self, each_slot_obj):
        each_user_slot_dto = UserSlotDto(
            slot_start_time=each_slot_obj.user_booked_slot__slot_start_time,
            slot_end_time=each_slot_obj.user_booked_slot__slot_end_time,
            slot_date=each_slot_obj.user_booked_slot__slot_end_time,
            washing_machine_id=each_slot_obj.user_booked_slot__washing_machine_id
        )
    ...
    def list_of_previous_slot_dtos(self, username: str, date: datetime.date):
        ...
        user_slot_objs = UserSlot.objects.filter(user_booked_slot__slot_date__lte=date)
        list_of_slot_dtos = self._get_list_of_user_slot_dto(user_slot_objs)
        return list_of_slot_dtos

    def validate_date(self, date) -> bool:
        from slot_booking.models.Slot import Slot, UserSlot
        date_valid = Slot.objects.filter(slot_date__gte=date).exists()
        if date_valid:
            return True
        else:
            return False
    ...
    def list_of_upcoming_slot_dtos(self, date: datetime.date, username: str):
        from slot_booking.models.Slot import Slot, UserSlot
        user_slot_objs = UserSlot.objects.filter(user_booked_slot__slot_date__gte=date).filter(user_slot_status="Booked")
        list_of_slot_dtos = self._get_list_of_user_slot_dto(user_slot_objs)
        return list_of_slot_dtos
